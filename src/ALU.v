...
module ALU(
    ...
    wire [31:0] internal_op_A = (mac_cmd == MULT32 && ~busy) ? accumulator[31:0] : 
    ...
    wire use_broadcast = (mac_cmd == MULT32 || busy);

    // Sign Control for op_A
    wire a_is_signed_msb = 1'b1; // The top byte (byteA3) is always signed
    wire a_is_signed_lower = (mac_cmd == MULT32 || busy) ? 1'b0 : 1'b1; // Lower bytes are unsigned during MULT32, but signed during MACs
    ...
    always @(posedge clk or negedge rst) begin
        if (!rst) begin
            ...
            if (mac_cmd == RST_ACC) begin
                accumulator <= 64'b0;
            end
            else begin
                    if (mac_cmd != MULT32 && ~busy) begin
                        done <= 1'b0; // Clear done flag for non-MULT32 commands
                    
                        if (mac_cmd == MAC8) begin
